...
export class UserService {
  ...
  findUser(query: string): Observable<string> {
    ...
    return this.http.get<string>(`http://localhost:8080/api/v1/users?q=${query}`, {
      params: httpParams
    });
  }
  ...
  createAccount(uname: string, pwd: string): Observable<HttpResponse<any>> {
    ...
    return this.http.post<HttpResponse<any>>(`http://localhost:8080/api/v1/users`, body, {
      observe: 'response'
    });
  }
  ...
  authenticate(uname: string, pwd: string): Observable<any> {
    ...
    return this.http.post(`http://localhost:8080/api/v1/auth`, body, {
      responseType: 'text'
    });
  }
}
